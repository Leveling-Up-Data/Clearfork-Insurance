...
const imgFallbackCover = encodeURI("/images/group photo 1 (1)_1761008519000.jpg");
const imgFallbackAvatar = encodeURI("/images/david hargrove head shot_1761004385331.jpg");
...
function RssEpisodeCard({ item, channelTitle }: { item: PodcastEpisode; channelTitle?: string }) {
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="p-6">
        ...
        <p className="mt-2 line-clamp-3 text-[15px] leading-[22px] text-muted-foreground">{item.excerpt}</p>
        <div className="mt-4 flex items-center gap-3">
          <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full bg-muted">
            <Image src={imgFallbackAvatar} alt="" fill className="object-cover" sizes="40px" />
          </div>
          <div>
            <div className="text-sm font-semibold text-foreground">{showLabel}</div>
            <div className="text-xs text-muted-foreground">{subtitle}</div>
          </div>
        </div>
        <audio controls preload="none" src={item.audioUrl} className="mt-4 h-10 w-full" />
      </div>
    </article>
  );
}
      ...
      <section className="bg-gradient-to-br from-primary/10 to-primary/5 py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          {props.mode === "rss" && props.channelTitle ? (
            <p className="mt-2 text-sm text-muted-foreground">RSS feed: {props.channelTitle}</p>
          ) : props.mode === "rss" ? (
            <p className="mt-2 text-sm text-muted-foreground">Latest episodes from our RSS feeds (newest first).</p>
          ) : null}
          {props.mode === "rss" && props.episodes.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">
              The feed URL is set but no playable episodes were returned. Check that the RSS includes audio
              enclosures and that the URL is reachable from the server.
            </p>
          ) : null}
        </div>
      </section>
        ...
        <PageShell>
          {props.mode === "rss" ? (
            props.episodes.length === 0 ? (
              <p className="text-center text-muted-foreground">No episodes to display.</p>
            ) : (
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8">
                {props.episodes.map((item) => (
                  <RssEpisodeCard key={item.id} item={item} channelTitle={props.channelTitle} />
                ))}
              </div>
            )
          ) : (
